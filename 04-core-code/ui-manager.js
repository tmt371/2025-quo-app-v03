...
export class UIManager {
  ...
  _renderQuickQuoteView(state) {
    console.log("[v0] Rendering UI, inputValue:", state.ui.inputValue)
    console.log("[v0] Input display element:", this.inputDisplay)

    if (this.inputDisplay) {
      this.inputDisplay.textContent = state.ui.inputValue || ""
      console.log("[v0] Set input display to:", this.inputDisplay.textContent)
    }
    ...
    if (this.resultsTableBody) {
      ...
      if (
        rollerBlindItems.length === 0 ||
        (rollerBlindItems.length === 1 && !rollerBlindItems[0].width && !rollerBlindItems[0].height)
      ) {
        this.resultsTableBody.innerHTML = `<tr><td colspan="5" style="color: #888;">Please enter dimensions to begin...</td></tr>`
      } else {
        this.resultsTableBody.innerHTML = rollerBlindItems
          .map((item, index) => {
